...
'no_sensors_cols': ['Unnamed: 0','drone', 'update_step', 'iter', 'script_time', 'sim_time','anomaly_type','scenario','original_iter','latitude', 'longitude', 'altitude','collision'],
...
train_X,train_labels, train_keys  =  data_to_signatures(drones_df=train_data,win_size_ls=config['win_size_ls'],no_sensors_cols=config['no_sensors_cols'])
val_X, val_labels , val_keys =  data_to_signatures(drones_df=val_data,win_size_ls=config['win_size_ls'],no_sensors_cols=config['no_sensors_cols'])
test_X, test_labels , test_keys =  data_to_signatures(drones_df=test_data,win_size_ls=config['win_size_ls'],no_sensors_cols=config['no_sensors_cols'])
...
val_data_for_collab_anomaly_detector = create_data_for_collaborative_anomaly_detection( val_keys, val_labels,val_MSCRED_RMSEs,config)
test_data_for_collab_anomaly_detector = create_data_for_collaborative_anomaly_detection(test_keys, test_labels,test_MSCRED_RMSEs,config)
...
val_keys_with_dtw_and_RMSE = agg_DTW_by_drone(val_dtw_between_drones,val_data_for_collab_anomaly_detector,drones_in_analysis=5)
test_keys_with_dtw_and_RMSE = agg_DTW_by_drone(test_dtw_between_drones,test_data_for_collab_anomaly_detector,drones_in_analysis=5)
...
DTW_PR_AUC = PR_AUC(test_keys_with_dtw_and_RMSE['label'].to_numpy(),test_keys_with_dtw_and_RMSE['DTW_dist'].to_numpy())
...
DTW_RMSE_PR_AUC = PR_AUC(test_keys_with_dtw_and_RMSE['label'].to_numpy(),test_keys_with_dtw_and_RMSE['DTW_RMSE'].to_numpy())


print('============RESULTS for {} =================='.format(config['scenario']))
print('MSCREDs_anomaly_score_PR_AUC: {} RMSE_PR_AUC: {} DTW_PR_AUC: {} DTW_RMSE_PR_AUC {} '.format(MSCREDs_anomaly_score_PR_AUC,RMSE_PR_AUC,MSCREDs_anomaly_score_PR_AUC,DTW_RMSE_PR_AUC))
